<?php
// REGOsymcon-Deploy -- ein Skript, das auf einem leeren Symcon alles aufbaut.
//
//   1. installiert die REGOsymcon-Modulbibliothek, falls sie fehlt
...
//   4. in jeden Raum die passenden REGOsymcon-Kacheln, verdrahtet mit den
...
const REGOSYMCON_REPOSITORY = 'https://github.com/epogo75/REGOsymcon';
// So heißt die Bibliothek in Symcons Modulverwaltung (der Ordnername).
const REGOSYMCON_BIBLIOTHEK = 'REGOsymcon';

// Modul-GUIDs der REGOsymcon-Bibliothek
const RGS_SCHALTEN = '{E5F57876-C2BE-4C9B-9D1E-237D9010ADA8}';
const RGS_DIMMEN   = '{A4507CF7-C921-467C-BD01-699C862B9F5C}';
const RGS_JALOUSIE = '{23F455EC-9236-480B-B02F-E10CE43DBDE2}';
const RGS_KLIMA    = '{FEB37553-F02A-4F1B-A669-15BCD71E0712}';
const RGS_SZENE    = '{C2314D3B-F6AD-40E2-B5B7-6DB850E0AD5E}';
const RGS_SENSOR   = '{0871A6F2-8912-4EC0-9C4F-616982DAFF34}';
const RGS_WETTER   = '{6EFCE386-425F-4AF5-8440-B93CAA0B3C2E}';
const RGS_INFO     = '{63561319-730C-4139-8F95-1DA3BD142C83}';
const RGS_TASTER   = '{2562CE14-21C3-4609-B04E-A9A69C51C684}';
const RGS_URL      = '{E72DF487-94FC-4942-8473-1FBEAF87564B}';
const RGS_ZAEHLER  = '{A9DEE307-F3B2-48A2-9960-245799F8BBD9}';
...
const KNX_GERAETE_IDENT = 'regosymcon_knx_geraete';
const INFO_IDENT        = 'regosymcon_info';
...
const KACHEL_PREFIX  = 'regosymcon_kachel_';
const MODBUS_PREFIX  = 'regosymcon_modbus_';
const MODBUS_ROOT_IDENT = 'regosymcon_modbus_root';
const LINK_PREFIX    = 'regosymcon_link_';
...
function ensure_module_installed()
{
    ...
    if (@IPS_GetModule(RGS_SCHALTEN) !== false) {
        // Schon da -- aber liegt auf GitHub etwas Neueres, wird es geholt.
        if (in_array(REGOSYMCON_BIBLIOTHEK, MC_GetModuleList($control[0]), true)
            && MC_IsModuleUpdateAvailable($control[0], REGOSYMCON_BIBLIOTHEK)) {
            MC_UpdateModule($control[0], REGOSYMCON_BIBLIOTHEK);
            $stand = MC_GetModuleRepositoryInfo($control[0], REGOSYMCON_BIBLIOTHEK);
            return 'aktualisiert auf ' . ($stand['ModuleCommit'] ?? 'neuen Stand');
        }
        return 'war schon installiert und aktuell';
    }

    MC_CreateModule($control[0], REGOSYMCON_REPOSITORY);

    for ($i = 0; $i < 30; $i++) {
        if (@IPS_GetModule(RGS_SCHALTEN) !== false) {
            return 'neu installiert von ' . REGOSYMCON_REPOSITORY;
        }
        IPS_Sleep(1000);
    }
...
function sync_infokachel($visuId, $aussentemperatur, &$index, &$visited)
{
    ...
    if (isset($index[INFO_IDENT])) {
        ...
        $id = IPS_CreateInstance(RGS_INFO);
 ...
 * Projekts (dessen Etagen sind damit die erste Ebene), und jede REGOsymcon-
...
function sync_modbus_zaehler($zaehler, $technikId, $kachelParentId, &$index, &$visited)
{
    ...
    if (isset($index[$kachelIdent])) {
        ...
        $kachelId = IPS_CreateInstance(RGS_ZAEHLER);
...
foreach ($tree['etagen'] as $etagePosition => $etage) {
    ...
    foreach ($etage['raeume'] as $raumPosition => $raum) {
        ...
        foreach ($raum['funktionen'] as $funktionPosition => $funktion) {
            ...
            if ($mapping['module'] === RGS_SENSOR) {
                $messwerte = array_merge($messwerte, array_values($werte));
            } elseif ($mapping['module'] === RGS_WETTER) {
                foreach ($zeilen as $zeile) {
            ...
            } elseif ($mapping['module'] === RGS_KLIMA) {
